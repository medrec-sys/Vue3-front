  ...
  >
    ...
    >
      ...
      <el-form-item label="维度">
        <el-input v-model="localVectorData.dim" placeholder="请输入维度" />
      </el-form-item>
...
const add = async () => {
  const res = await vectorStore.createVector(localVectorData.value)
  if (res.code === 1) {
...
const update = async () => {
  const res = await vectorStore.updateVector(localVectorData.value)
  if (res.code === 1) {
